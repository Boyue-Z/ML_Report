...
for idx in range(1, 562):
    X_train_sub = np.delete(X_train, range(idx,562), axis=1)
    X_test_sub = np.delete(X_test, range(idx,562), axis=1)
    print("Level:", idx)
    # print(X_train_sub.shape)
    # print(X_test_sub.shape)

    linear_svm = LinearSVC().fit(X_train_sub, y_train)
    training_score = linear_svm.score(X_train_sub, y_train)
    testing_score = linear_svm.score(X_test_sub, y_test)
...
plt.plot(need_idx, training_scores, 'b-')
plt.plot(need_idx, testing_scores, 'r--')
